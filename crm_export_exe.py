...
def parse_leads_from_report_html(content_html):
    ...
    for row in rows:
        ...
        phone_match = re.search(r"\+?\d[\d\s()\-]{7,}\d", joined_text)
        if not phone_match:
            tel_match = re.search(r"href=[\"']tel:([^\"']+)[\"']", row, flags=re.IGNORECASE)
            if tel_match:
                phone_match = re.search(r"\+?\d[\d\s()\-]{7,}\d", tel_match.group(1))

        if not phone_match:
            attr_match = re.search(r"(?:data-phone|data-value|value)=[\"']([^\"']*\+?\d[^\"']*)[\"']", row, flags=re.IGNORECASE)
            if attr_match:
                phone_match = re.search(r"\+?\d[\d\s()\-]{7,}\d", attr_match.group(1))

        if not (lead_id or name_value or email_match or phone_match):
            continue
